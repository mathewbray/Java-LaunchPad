...
param (
        ...
        [Switch]$EndJob,
        [Switch]$Query,
        $KeyToPress = '{F15}' # Default KeyPress is <Ctrl>
...
#-- INITIAL PROMPTS
    ...
    $sleepseconds = Read-Host -Prompt 'Seconds between automatic key press (Default: 30) '

#-- DEFAULTS IF BLANK
if($keepalivehours -eq "") {
Write-Host "Using default keep alive: 9" -ForegroundColor Yellow
$keepalivehours = '9'
}
 
if($sleepseconds -eq ""){
Write-Host "Using default seconds between key press: 30" -ForegroundColor Yellow
$sleepseconds = '30'
}

    

Start-KeepAlive -KeepAliveHours $keepalivehours -SleepSeconds $sleepseconds
