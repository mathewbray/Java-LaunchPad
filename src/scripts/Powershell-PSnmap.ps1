...
# INITIAL MENU
    #-- Hostname, IP, or Subnet (172.16.0.1/24) 
        $computersToScan = Read-Host -Prompt 'Hostname, IP, or Subnet (172.16.0.1/24) '
        if($computersToScan -eq $null){ $computersToScan = "127.0.0.1" }
    ...
    #  Output Type
        $Title = "Resolve DNS?"
        $Info = "Do you want to resolve DNS?"
...
Invoke-Item "$DesktopPath\Logging-Output\PortScan-$TimeStamp.csv"
